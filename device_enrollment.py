...
@contextmanager
def _connect_path(path: str | Path):
...
def _init_tables(db_path: Path) -> None:
    with _connect_path(db_path) as db:
        db.execute("PRAGMA foreign_keys = ON")
        db.execute(
            """
...
def _claim_status_payload(row: sqlite3.Row, *, user_id: int, now: int) -> dict[str, Any]:
    ...
    if status == "rejected":
        # This status is visible only to the logged-in owner of this specific
        # one-time claim. The browser never supplies or probes a device_id, so
        # returning the definitive claim outcome does not create a device-ID
        # enumeration path.
        error_code = str(row["rejection_reason"] or "claim_rejected")
        result["error_code"] = error_code
        if error_code == "device_already_owned":
            result["error"] = "This FloraCore is already linked to another account."
        else:
            result["error"] = "The device could not be linked to this account."

    if status == "expired":
        result["error_code"] = "claim_expired"
        result["error"] = "This Connection Code is no longer valid."

    if status == "cancelled":
        result["error_code"] = "claim_cancelled"
        result["error"] = "This Connection Code is no longer active."
